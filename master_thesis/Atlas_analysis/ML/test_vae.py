...
class VariationalEncoder(nn.Module):
    def __init__(self, latent_dims, input_shape):
        ...
        self.linear1 = nn.Linear(input_shape, 1000)
...
class Decoder(nn.Module):
    def __init__(self, latent_dims, input_shape):
        ...
        self.linear2 = nn.Linear(1000, input_shape)
        self.dropout = nn.Dropout(.15)
...
def main():
    ...
    for train, test in skf.split(X=data, y=targets):
        ...
        vae_test = VariationalAutoencoder(50, input_shape)
        ...
        train_oversample_encoded, y_train_oversample = SMOTETomek(smote=SMOTE(k_neighbors=4)).fit_resample(train_encoded, Y_train)
        ...
        for model in models:
            ...
            results_df = pd.DataFrame({"model": [type(model).__name__], "fold": [fold], "micro_f1": [micro_f1],
                                            ...
                                            "oversampler": ["VAE/SMOTETomek"], "splitter": ["skf"]})
                
            uml.save_results(results_df, "VAE_evaluation_2") 
    ...
    # For complete project split
    print("Whole project split...")

    models = [LogisticRegression(max_iter=10000), SVC(), RandomForestClassifier()]
    skf = uml.ProjectBasedSplit(10, meta, on="Group")
    ...
    for train, test in skf.split(X=data, y=targets):
        ...
        for model in models:
            ...
            results_df = pd.DataFrame({"model": [type(model).__name__], "fold": [fold], "micro_f1": [micro_f1],
                                            "macro_f1": [macro_f1], "weighted_f1": [weighted_f1] ,"cm": [cm], "prediction": ["original"],
                                            "oversampler": ["VAE/SMOTETomek"], "splitter": ["pxd"]})
