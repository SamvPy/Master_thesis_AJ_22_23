...
class LowestValueImputerGaussian(BaseEstimator, TransformerMixin):
    '''
    Imputes by sampling from left shifted Gaussian distribution.
    '''
...
class MNAR_MCAR_Imputer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=np.array([None])):
        ...
        # else test set must be fitted. Retrain PCA/KNN together with imputed training data
            ...
            self.test_mv_indices = melted_train_and_test.index[melted_train_and_test.isna()["value"]]
            ...
            self.X_pca_imputed_test = self.X_pca_imputed_test.loc[test_indices, :].reset_index(drop=True)
            self.X_knn_imputed_test = pd.DataFrame(self.X_knn_imputed_test).loc[test_indices, :].reset_index(drop=True)

            assert len(self.X_pca_imputed_test) == len(X)
...
class FeatureSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        for selector in self.selectors:
            ...
            if selector == "LR":
                self.selector_models.append(SelectFromModel(LogisticRegression(class_weight = "balanced", penalty="l1", solver = "liblinear", multi_class="ovr"), max_features=self.num_features))
            if selector == "SVC":
                self.selector_models.append(RFE(SVC(class_weight="balanced", kernel = "linear"), n_features_to_select=self.num_features, step=50))
            if selector == "RF":
                self.selector_models.append(RFE(RandomForestClassifier(class_weight="balanced"), n_features_to_select=100, step=50))
